...
*Start the Macro;
*Continuous variables in this data is divided into tertiles (3 groups), if you would like to change it to other groups, modify the group in proc rank;
...
%let var&i.=%scan(&contvar.,&i.);
 ...
 proc sort data = &data.;
   ...
   create table out_&i. as
   select
   cat(estimate,' (',cat(LowerLimit, ', ', UpperLimit),')') as Median
   from _q_&i.;
   quit;
...
%macro con (data=t51, time=lf, censor=fstat, catevar=gender cvd afb sho chf av miord mitype, contvar=age hr sysbp diasbp bmi, datout=demo);
